...
You can use the `geom_text_contour` function from the `metR`package to add labels to the contour lines.

```{r}
library(metR)
p_depth_contour_labels <- p_depth_contour +
  geom_text_contour(data = depth,
                    aes(x = x, y = y, z = z),
                    breaks    = c(-50, -100, -200, -400, -1000),
                    colour    = "white",
                    size      = 1,
                    stroke    = 0.2,        # thin dark outline improves legibility
                    rotate    = TRUE)

p_depth_contour_labels
```
...
(such as ICES rectangles). The helper function below bins coordinates to a grid and returns the cell midpoint:
