...
library(metR)         # Includes contour labelling
```
...
After saving it, you can embed the plot in a Quarto or R Markdown document.
...
p_base <- ggplot(world, aes(long, lat, group = group)) +
  ...
  xlim(xlim_demo) + # display crop only — see the discussion in the Layers section below
  ylim(ylim_demo) +
  theme_bw() +
...
You can use `geom_text_contour()` from the `metR` package to add labels to the contour lines.

```{r}

p_depth_contours <- p_depth +
  geom_contour(data = depth, aes(x = x, y = y, z = z),
               breaks = c(-50, -100, -200, -400, -1000),
               linewidth = 0.3,
               colour    = "white") +
  geom_text_contour(data = depth, aes(x = x, y = y, z = z),
               breaks = c(-50, -100, -200, -400, -1000),
                    colour    = "white",
                    size      = 2.5,
                    stroke    = 0.05,        # thin dark outline improves legibility
                    rotate    = TRUE)        # rotate labels to follow the contour
