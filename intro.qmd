...
This course uses **terra** for raster data. Some chapters also reference the older **raster** package, which you may encounter in existing code. The two packages have similar functionality but different syntax. If you see a function like `raster()` or `stack()`, it is from the raster package; the terra equivalents are `rast()` and `c()`.
:::
