...
```{r}
st_geometry_type(bor)   # Geometry type of each feature
st_crs(bor)             # Full CRS information
st_crs(bor)$Name        # CRS name
st_crs(bor)$epsg        # EPSG code (if defined)
st_bbox(bor)            # Bounding box
...
```{r}
plot(bor)               # One panel per attribute
plot(st_geometry(bor))  # Geometry only
```
...
stations_with_data <- stations_sf %>%
  left_join(smb %>% select(id, cod_n, haddock_n), by = "id")
```
