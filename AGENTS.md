...
All datasets live in `data/`. They are available for download from: `https://raw.githubusercontent.com/jmburgos/TCMSAR26/main/data/<filename>`
...
- **`rasters.qmd` line 66**: reads `"./data/Iceland_btemp.tif"` — file does not exist. Correct path is `"./data/Iceland_minbtemp.tif"`. Will cause a render error when `execute: enabled` is turned on for that file.
...
Several files that exist in `data/` are still read via `ftp://ftp.hafro.is/...` in some chapters (`iceland_coastline.gpkg`, `helcom.gpkg`, all raster TIFFs in `rasters.qmd`, `is_smb_stations.csv` in `rasters.qmd` and `interpolation.qmd`, `is_smb_biological.csv` in `interpolation.qmd`). These chapters will fail offline even though the files are in `data/`.

## Content audit

See [`content_notes.md`](content_notes.md) for: - Schedule sequencing issues (`crs.qmd` taught after chapters that require it) and a proposed alternative schedule (CRS + ggplot2 on Tuesday, geometric moves to Wednesday) - `ggplot2.qmd` content structure: sf-only vector mapping; raster visualisation split to `rasters.qmd` ("Rasters and ggplot" section already present) - Full content overlap analysis across chapters - Complete data usage tally (which dataset appears in which chapter)
